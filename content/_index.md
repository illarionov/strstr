...
и Домиником Перреном (Dominique Perrin) в 1991 году.[²](#reference_twoway_string_matching)
...
случаях $p$ будет периодом строки $x$:
   ...
   1) Возьмем некоторые строки $y$, $z$ равной длины и строку $w$, такие, что
     ${x=yw=wz}$; ${|y|=|z|}$. В этом случе длина $y$ и $z$ будет периодом строки
     $x$: ${|y|=|z|=p}$.  
     __Пример:__  
      ${x=\mathrlap{
...
:   *Лексикографический* (или алфавитный) порядок определяется на множестве слов
${A^{*}}$ упорядоченного алфавита $A$. Для двух слов ${x, y ∈ A^\*}$ слово ${x≤y}$ тогда, когда
выполняется одно из условий:
    * $x$ — префикс $y$
    * $x$ *строго меньше* $y$. Т. е. $x$ и $y$ можно представить как
    ${x=w\texttt{A}x, y=w\texttt{B}y}$, где ${w,x,y}$ — слова  из множества ${A^{*}}$; а
...
${A=\\{\texttt{A},\texttt{B}\\}}$ с ${\texttt{A} < \texttt{B}}$. Для слов ${\tt ABB}$ и
...
${|f|=per(Maxsuf(x))}$, т.е. длина $f$ — это период строки макс. суффикса;  
